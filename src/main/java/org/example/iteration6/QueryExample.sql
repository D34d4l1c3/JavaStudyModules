...
select count(b.id) as pop_bc, b.id, max(b.name),b.type as bk_name
        from library.rent_log r inner join library.product_unit pu on pu.id = r.product_id
...
with popular_rent as (select r.id  as rent_id,pu.id  as unit_id,pu.book_id, a.name,
                             b.name as bk_name,ba.author_id,r.start_date,r.till_date,r.status
                      from library.rent_log r
...
where r.status ='Возвращено' or (r.status='Отдано' and r.till_date >= current_date)
group by client_id order by cnt_cl_per_rent desc limit 100;
...
from library.rent_log r
         inner join library.client_card cc on cc.id = r.rent_by_id
         left join library.clients c on c.id = cc.client_id
where r.status ='Потеряно' or (r.status='Отдано' and r.till_date < current_date)
group by client_id order by cnt_cl_per_rent desc limit 100;
...
select start_date as event_day,count(id) ,'Отдано' as stat from rent_month t where t.status = 'Отдано' group by start_date
union all
select return_date as event_day,count(id),'Возвращено' as stat from rent_month t where t.status = 'Возвращено' group by return_date
